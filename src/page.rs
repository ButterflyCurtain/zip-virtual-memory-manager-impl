...
//! （read-ahead）を本キャッシュに充填する。Diff Layer はまだ無いので、現状の
//! 読み取りは「ページキャッシュ → シーク索引 + ソース ZIP」の 2 段。
//!
...
//! バウンドし、超過時は末尾（最も古い）から退避する。`madvise`/COW dirty 追跡は
//! まだ無い（COW は Diff Layer = M2 以降）。
